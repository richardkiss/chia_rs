...
def solution_generator_2026(spends: Sequence[tuple[Coin, bytes, bytes]]) -> bytes: ...
...
def generator_interned_weight(program: ReadableBuffer) -> int: ...
...
def tree_hash_auto(blob: ReadableBuffer) -> bytes32: ...
...
@final
@disjoint_base
class Block2026Builder:
    # Anytime block builder for post-HF2 (INTERNED_GENERATOR).
    # Usable as a context manager: ``__exit__`` calls ``close()``.
    ...
    def add_candidate(self, bundle: SpendBundle, irreducible_cost: uint64) -> None: ...
    def start(self) -> None: ...
    def best(self) -> tuple[bytes, G2Element, uint64, list[int]]: ...
    def close(self) -> None: ...
